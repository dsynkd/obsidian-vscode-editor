...
## 关于插件
Obsidian 默认不支持对各种代码文件的查看和编辑，而 Obsidian Code Editor 就是为了解决这个问题而生的。

它基于 [Monaco Editor](https://microsoft.github.io/monaco-editor/)（与 Visual Studio Code 使用同源编辑器组件）开发，支持对各种代码格式文件的查看和编辑，包括但不限于：`C`、`C++`、`C#`、`CSS`、`Go`、`HTML`、`Java`、`JavaScript`、`JSON`、`Python`、`Ruby`、`Rust`、`Shell`、`XML`、`YAML`、`ini` 等。

你再也不用为了查看或编辑某个代码文件而打开另一个编辑器了，一切都在 Obsidian 中完成。

## 基本特性
* **不依赖**任何的第三方的网络服务，未联网也能使用
- 支持对各种代码格式文件的查看和编辑，体验接近本地代码编辑器
- 支持的编辑器主题：浅色主题、深色主题
...
![image.png](https://github.com/sunxvming/obsidian-code-editor/blob/main/img/20231103103939.png)


## 支持的快捷键
大部分的快捷键与 Visual Studio Code 保持一致，以下是一些支持的快捷键
...
## 代码块编辑的支持
 ...
 只需**右键单击**任何代码块，然后单击“**在 Code Editor 中编辑代码块**”。
...
![image.png](https://github.com/sunxvming/obsidian-code-editor/blob/main/img/20231103104141.png)
...
**手动安装**:
1. 进入最新版本的github下载页面 [latest release](https://github.com/sunxvming/obsidian-code-editor/releases/latest)
2. 下载 `obsidian-code-editor-x.y.z.zip` 文件,其中 `x.y.z` 是版本号
3. 解压并提取 obsidian-code-editor 文件夹，然后放到你 Obsidian 库中的插件目录中 `<库>/.obsidian/plugins/` (注意: `.obsidian` 文件夹可能被隐藏了，你需要先将该文件夹展示出来)
4. 打开设置 > 第三方插件，重新加载和激活该插件

## 联系和反馈

如果你在使用该插件过程中，遇到各种问题、或有什么好的建议，欢迎在 [GitHub issues](https://github.com/sunxvming/obsidian-code-editor/issues) 中提出。
